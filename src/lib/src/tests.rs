pub mod tests {
    ...
    use crate::queue_readers::RotatingReadBuffers;
    ...
    fn push_messages() {
        ...
        assert_eq!(
            val.is_none(),
            true,
            "The message has been read. There are none left. Should be none"
        );
        ...
        assert_ne!(
            val.is_some(),
            true,
            "Another message has been added. Should be some"
        );
    }
    ...
    fn raw_receive() {
        ...
        let mut readers: RotatingReadBuffers<MmqpNormalisedMessage> =
            RotatingReadBuffers::new(4, 65536);
        ...
        assert_eq!(
            v.len(),
            raw.len(),
            "Read raw length should be same as original raw length"
        );
    ...
    fn decode_string() {
        ...
        let second: String = "this string is appended to the previous on the packed binary format"
            .to_string()
            .repeat(55)
            .repeat(13);
        ...
        println!(
            "length: {:?}",
            first.clone().to_mmqp_binary().unwrap().len()
        );
        ...
        assert_eq!(
            string2.to_mmqp_binary().unwrap().len(),
            string2.mmqp_binary_size()
        );

        assert_eq!(first, string2, "The first string is not properly extracted");
        assert_eq!(
            string3, second,
            "The first string is not properly extracted"
        );
        assert_eq!(
            cursor,
            bytes.len(),
            "The cursor is not at the end of the second string after extracting both"
        );
    }
